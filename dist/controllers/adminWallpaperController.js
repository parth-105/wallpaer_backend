...
export async function triggerAutoImport(req, res, next) {
    try {
        const { pexelsPhotos, pexelsVideos, pixabayPhotos, pixabayVideos, wallhavenPhotos } = req.body || {};
        const config = {};
        if (pexelsPhotos != null)
            config.pexelsPhotos = Number(pexelsPhotos);
        if (pexelsVideos != null)
            config.pexelsVideos = Number(pexelsVideos);
        if (pixabayPhotos != null)
            config.pixabayPhotos = Number(pixabayPhotos);
        if (pixabayVideos != null)
            config.pixabayVideos = Number(pixabayVideos);
        if (wallhavenPhotos != null)
            config.wallhavenPhotos = Number(wallhavenPhotos);
