 ...
 */
export const getVersionStatus = async (_req, res) => {
    try {
 ...
 */
export const getAdminVersionConfig = async (_req, res) => {
    try {
        const configDoc = await AppConfig.findOne({ key: 'version_config' }).lean();
        if (configDoc && configDoc.versionConfig) {
            const cfg = configDoc.versionConfig;
 ...
 */
export const updateAdminVersionConfig = async (req, res) => {
    try {
        const { minVersionCode, preReleaseBlocked, playStoreUrl, message } = req.body;
        const updatedConfig = await AppConfig.findOneAndUpdate({ key: 'version_config' }, {
            $set: {
                key: 'version_config',
                versionConfig: {
